...
test("an incident can be deleted", async()=>{
  ...
  try {
    ...
    assert.equal(await store.deleteIncident("FLR-2026-0001"),true);
    assert.equal(await store.getIncident("FLR-2026-0001"),null);
    assert.equal(await store.deleteIncident("FLR-2026-0001"),false);
  } finally { await rm(dir,{recursive:true,force:true}); }
});
