...
export function createProjectContract(project,input={},now=Date.now()){
 const sourceContract=input.contract||input;
 const explicitVersion=input.version??sourceContract.version;
 const contract=validateConnectivityContract(sourceContract);
 const existing=catalog(project).filter(item=>item.contract.id===contract.id);
 const version=explicitVersion!=null?Number(explicitVersion):Math.max(0,...existing.map(item=>item.contract.version))+1;
...
export function publishProjectContract(project,entryIdValue,now=Date.now()){let published;const updated=mutateEntry(project,entryIdValue,entry=>{if(entry.status!=="draft")throw new Error("Only draft contracts can be published.");published={...entry,status:"published",publishedAt:nowIso(now),updatedAt:nowIso(now)};return published;});return {project:{...updated,updatedAt:nowIso(now)},entry:published};}
export function deprecateProjectContract(project,entryIdValue,now=Date.now()){let deprecated;const updated=mutateEntry(project,entryIdValue,entry=>{if(entry.status!=="published")throw new Error("Only published contracts can be deprecated.");deprecated={...entry,status:"deprecated",deprecatedAt:nowIso(now),updatedAt:nowIso(now)};return deprecated;});return {project:{...updated,updatedAt:nowIso(now)},entry:deprecated};}
export function cloneProjectContractVersion(project,entryIdValue,input={},now=Date.now()){const source=catalog(project).find(item=>item.id===entryIdValue);if(!source){const e=new Error("Contract catalog entry was not found.");e.statusCode=404;throw e;}const nextContract={...structuredClone(source.contract),...(input.contract||{})};delete nextContract.version;return createProjectContract(project,{...input,contract:nextContract,source:input.source||`cloned-from:${source.id}`,referenceUrl:input.referenceUrl??source.provenance?.referenceUrl,notes:input.notes||`New draft from ${source.contract.id} v${source.contract.version}.`},now);}
