...
import { createDemoRouter } from "./demo/routes.mjs";
import { capabilities, isHosted, isPublicDemo } from "./runtime/capabilities.mjs";
...
const hostedRuntime = isHosted();
const publicDemoRuntime = isPublicDemo();

// Generating a development credential is a convenience for `npm start` on a
// laptop. On a hosted deployment it is a liability: the generated value would
// be printed to a platform log, and an operator could reasonably assume the
// control plane was configured when it was not. A hosted runtime with no
// configured token therefore gets an UNUSABLE credential - a random value that
// is never printed and that nobody holds - so the admin surfaces stay reachable
// only to whoever sets FAULTLINE_ADMIN_TOKEN.
const generatedAdminToken = configuredAdminToken ? null : generateCredential("fl_admin");
const adminTokenHash = hashCredential(configuredAdminToken || generatedAdminToken);
const adminCredentialConfigured = Boolean(configuredAdminToken);
...
/**
 * Serve an HTML document with this deployment's runtime stamped onto <html>.
 *
 * The frontend has exactly one runtime question - hosted public demo, or an
 * operator's own control plane - and the answer decides which surface it lands
 * on and what a measurement may be labelled. Fetching that answer would mean a
 * first paint made with the wrong assumption, so it travels in the markup.
 *
 * The substitution is on a fixed attribute string rather than a template
 * placeholder, so the file on disk is a valid standalone document and the
 * local-install defaults are what you get if it is opened straight from the
 * filesystem. Every value written here is generated by Faultline from its own
 * environment; none of it comes from the request.
 */
function sendHtml(res, html) {
  const runtime = capabilities();
  const attributes = [
    `data-runtime="${runtime.runtime}"`,
    `data-public-demo="${runtime.publicDemo ? "true" : "false"}"`,
    `data-vantage-label="${escapeAttribute(runtime.vantage.label)}"`,
    `data-vantage-region="${escapeAttribute(runtime.vantage.region || "")}"`
  ].join(" ");
...
// Unauthenticated by design, and mounted only when this deployment IS a public
// demo. See src/demo/routes.mjs for what makes that safe.
const handleDemo = createDemoRouter({ bodyFrom, json });

const server = createServer(async (req, res) => {
  try {
    const url = new URL(req.url, `http://${req.headers.host || "localhost"}`);

    // First, so a public-demo path is never shadowed by an admin router.
    if (await handleDemo(req, res, url)) return;
    ...
    // The one runtime description the frontend reads. Every panel asks this
    // rather than each module inventing its own hosted/local detection.
    // Public: it describes what this deployment can observe, never a secret,
    // and deliberately does not reveal whether an admin credential is set.
    ...
    if (req.method === "GET" && url.pathname === "/api/health") {
      return json(res, 200, {
        ...
        publicDemo: publicDemoRuntime,
        persistence: !hostedRuntime,
        ...
        networkBisect: true,
        localAnalyst: !hostedRuntime,
        flightRecorder: true,
        incidentCapsules: true,
        recorderIncidentPersistence: process.env.FAULTLINE_RECORDER_PERSIST !== "0" && !hostedRuntime
      });
    }
    ...
    try {
      ...
      if (extname(filePath) === ".html") return sendHtml(res, data.toString("utf8"));
      res.writeHead(200, { "content-type": mime[extname(filePath)] || "application/octet-stream" });
      res.end(data);
    } catch {
      sendHtml(res, await readFile(join(root, "index.html"), "utf8"));
    }
  } catch (error) {
...
server.listen(port, () => {
  ...
  console.log(`Runtime: ${capabilities().runtime}${publicDemoRuntime ? " · public demo enabled" : ""}`);
  console.log(`Persistent store: ${dataFile}${hostedRuntime ? " (ephemeral on this runtime)" : ""}`);

  if (adminCredentialConfigured) return;

  if (hostedRuntime) {
    // Never print a credential on a hosted runtime: platform logs are not a
    // secret channel, and a printed value is a published value.
    console.log("No FAULTLINE_ADMIN_TOKEN is configured. Admin and operator APIs are unreachable on this deployment.");
    console.log("Set FAULTLINE_ADMIN_TOKEN in the hosting environment to enable them. The public demo does not need it.");
    return;
  }

  console.log("No FAULTLINE_ADMIN_TOKEN was configured. Generated a development admin credential:");
  console.log(generatedAdminToken);
