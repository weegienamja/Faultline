...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--no-deep") options.deep = false;
...
async function fetchSession(base, sessionId, token) {
  const response = await fetch(`${base}/api/sessions/${encodeURIComponent(sessionId)}`, { headers: { authorization: `Bearer ${token}` }, cache: "no-store" });
...
async function main() {
  ...
  if (options.sessionId) {
    ...
    help(); process.exitCode = 1; return;
  } else if (!options.dryRun) throw new Error("Uploading endpoint evidence requires an authenticated session.");
  ...
  if (options.deep !== false) {
    const deep = await collectDeepDiagnostics(payload.telemetry.target);
    payload.telemetry.deepDiagnostics = deep;
    Object.assign(payload.metrics, {
      ipv4Reachable: deep.summary.ipv4Reachable,
      ipv6Reachable: deep.summary.ipv6Reachable,
      tlsHandshakeOk: deep.summary.tlsOk,
      tlsHandshakeMs: deep.summary.tlsHandshakeMs,
      targetTtfbMs: deep.summary.httpTtfbMs,
      pathMtuBytes: deep.summary.pathMtuBytes
    });
  }
  if (session) payload.sessionId = session.id;
  const m = payload.metrics;
  const topology = payload.telemetry?.topology;
  const deep = payload.telemetry?.deepDiagnostics;
  ...
  if (deep) {
    console.log(`  IPv4/IPv6: ${deep.summary.ipv4Reachable === true ? "v4 OK" : deep.summary.ipv4Available ? "v4 fail" : "v4 n/a"} · ${deep.summary.ipv6Reachable === true ? "v6 OK" : deep.summary.ipv6Available ? "v6 fail" : "v6 n/a"}`);
    if (deep.tls) console.log(`  TLS: ${deep.tls.ok ? deep.tls.protocol || "OK" : `failed (${deep.tls.error})`} · ${deep.tls.elapsedMs} ms`);
    if (deep.http?.timings?.ttfbMs != null) console.log(`  HTTP TTFB: ${deep.http.timings.ttfbMs} ms`);
    if (deep.pathMtu?.pathMtuBytes) console.log(`  Path MTU: ${deep.pathMtu.pathMtuBytes} bytes`);
  }
  console.log(`  Target ICMP loss: ${m.upstreamLoss}% · jitter ${m.jitterMs} ms`);
  console.log(topology ? `  Topology: ${topology.kind} · ${topology.confidence} confidence · ${topology.nodes.length} nodes` : "  Topology: skipped");

  if (options.json || options.dryRun) console.log(JSON.stringify(payload, null, 2));
  if (options.dryRun) { console.log("Dry run complete. Nothing was uploaded."); return; }
  ...
  if (session?.assignedProbeId) console.log(`Registered probe ${session.assignedProbeId} can now discover this session from its job queue.`);
  else console.log("Run the one-off remote-probe command issued when the diagnostic session was created to add the second vantage point.");
}

main().catch(error => { console.error(`Faultline agent failed: ${error.message}`); process.exitCode = 1; });
