...
async function main() {
  ...
  const intervalMs = Math.min(Math.max((Number(args.flags.interval) || 3) * 1000, 2_000), 30_000);
  ...
  const recorder = createRecorder({
    ...
    intervalMs,
    windowMs,
    deepCapture: args.flags["no-deep-capture"] ? null : createDeepCapture(),
    ...
    onEvent: event => {
      if (event.type === "sample") console.log(renderSample(event.sample));
        ...
        if (incident) console.log(renderIncident(incident));
      }
    }
  });
  ...
  const shutdown = () => {
    ...
    if (incident && !process.env.FAULTLINE_RECORDER_QUIET) {
